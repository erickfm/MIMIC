#!/usr/bin/env bash
# Kill the slow serial reshard and restart it with the parallel path
# (--shard-procs 96), reading the already-staged full master-Fox .slp, then
# train. Staging (102,301 .slp in mastfox_full_slp) is already done.
set -uo pipefail
cd /workspace/MIMIC
...
echo "[KILL $(date +%H:%M)] stop any running reshard (python procs only) + wrappers"
ps -eo pid,comm,args | grep slp_to_shards | grep -v grep | awk '$2 ~ /python/ {print $1}' | xargs -r kill -9 || true
pkill -f reshard_serial_box.sh 2>/dev/null || true
pkill -f build_full_masterfox_box.sh 2>/dev/null || true
sleep 5
echo "  remaining slp_to_shards python: $(ps -eo comm,args | grep -c '[s]lp_to_shards.py')"

echo "[CLEAN $(date +%H:%M)] remove partial shards"
rm -f "$DATA"/*.pt "$DATA/tensor_manifest.json" 2>/dev/null || true
[ -f "$DATA/mimic_norm.json" ] || { echo "ABORT: no mimic_norm.json in $DATA"; exit 3; }
echo "  staged .slp: $(find "$STAGE" -maxdepth 1 -name '*.slp' | wc -l)"

echo "[RS $(date +%H:%M)] PARALLEL reshard (96 procs)"
python3 tools/slp_to_shards.py --slp-dir "$STAGE" --meta-dir "$DATA" \
  ...
  --shard-gb 1.0 --val-frac 0.1 --seed 42 --shard-procs 64
...
echo "FULL_MASTERFOX_PAR_FINISHED $(date +%H:%M)"
